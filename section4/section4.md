...
else の次の行からインデントして書いた`if`文は，`else` 後の処理になります．
`score`が60点未満で不合格の場合，出席していれば情けで合格，出席もしていなければ落単になります．
このように，`if`文の中に`if`文を入れることで，複雑な条件分岐を表現することができます．
